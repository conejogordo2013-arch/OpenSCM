...
static int vm_ffi_signature_from_text(const ScmlValue *return_value, const ScmlValue *arg_value, ScmlFFISignature *signature, ScmlFFIType *arg_types, size_t max_arg_types) {
    if (!return_value || !arg_value || !signature || return_value->type != SCML_VAL_STRING || arg_value->type != SCML_VAL_STRING) return 0;
    signature->return_type = scml_ffi_parse_return_type(return_value->string, SCML_FFI_TYPE_INT32);
    signature->arg_count = 0;
    signature->arg_types = NULL;
    if (arg_value->string && arg_value->string[0]) {
        size_t parsed = scml_ffi_parse_arg_types(arg_value->string, arg_types, max_arg_types);
        if (parsed > max_arg_types) return 0;
        signature->arg_count = parsed;
        signature->arg_types = parsed ? arg_types : NULL;
    }
    return 1;
}

static int vm_ffi_call_pointer_args(const ScmlValue *fn_value, const ScmlValue *return_value, const ScmlValue *arg_value, const ScmlValue *call_args, size_t call_arg_count, ScmlFFIAbi abi, ScmlValue *ret) {
    ScmlFFIType arg_types[SCML_OPERANDS_MAX];
    ScmlFFISignature signature;
    ...
    return scml_ffi_call_native_abi((void *)vm_value_to_pointer(fn_value), call_args, &signature, abi, ret);
}

static int vm_ffi_call_name_args(const ScmlValue *name_value, const ScmlValue *return_value, const ScmlValue *arg_value, const ScmlValue *call_args, size_t call_arg_count, ScmlFFIAbi abi, ScmlValue *ret) {
    if (!name_value || name_value->type != SCML_VAL_STRING) return 0;
    ScmlFFIType arg_types[SCML_OPERANDS_MAX];
    ScmlFFISignature signature;
    if (!return_value && !arg_value) {
        const ScmlFFISignature *declared = scml_ffi_get_declared_signature(name_value->string);
        if (!declared || declared->arg_count != call_arg_count) return 0;
        return scml_ffi_call_native_by_name_abi(name_value->string, call_args, declared, abi, ret);
    }
    if (!vm_ffi_signature_from_text(return_value, arg_value, &signature, arg_types, sizeof(arg_types) / sizeof(arg_types[0]))) return 0;
    if (signature.arg_count != call_arg_count) return 0;
...
static int vm_ffi_builtin(const char *name, const ScmlValue *args, size_t arg_count, ScmlValue *ret) {
    ...
    if (strcmp(name, "ffi.declare") == 0 || strcmp(name, "ffi.declare_abi") == 0) {
        if ((arg_count != 3 && arg_count != 4) || args[0].type != SCML_VAL_STRING || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        if (arg_count == 4 && args[3].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_declare_function_text_abi(args[0].string, args[1].string, args[2].string, arg_count == 4 ? args[3].string : "default");
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.bind") == 0 || strcmp(name, "ffi.bind_name") == 0) {
        if ((arg_count != 5 && arg_count != 6) || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING || args[4].type != SCML_VAL_STRING) return 0;
        if (arg_count == 6 && args[5].type != SCML_VAL_STRING) return 0;
        void *handle = (void *)vm_value_to_pointer(&args[0]);
        int ok = scml_ffi_bind_function_text(args[1].string, handle, args[2].string, args[3].string, args[4].string, arg_count == 6 ? args[5].string : "default");
        if (ret) *ret = value_int(ok);
        return ok;
    }
    ...
    if (strcmp(name, "ffi.resolve") == 0 || strcmp(name, "ffi.find_symbol") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        void *symbol = scml_ffi_get_bound_symbol(args[0].string);
        if (!symbol) symbol = scml_ffi_find_symbol(args[0].string);
        if (!symbol) return 0;
        if (ret) *ret = value_pointer((uintptr_t)symbol);
        return 1;
    }
    ...
    if (strcmp(name, "ffi.call_abi_ptr") == 0 || strcmp(name, "ffi.call_ptr_abi") == 0) {
        if (arg_count < 4 || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING) return 0;
        ScmlFFIAbi abi = scml_ffi_parse_abi(args[1].string, SCML_FFI_ABI_DEFAULT);
        return vm_ffi_call_pointer_args(&args[0], &args[2], &args[3], args + 4, arg_count - 4, abi, ret);
    }
    if (strcmp(name, "ffi.call_name") == 0 || strcmp(name, "ffi.call_by_name") == 0) {
        if (arg_count < 1 || args[0].type != SCML_VAL_STRING) return 0;
        if (arg_count >= 3 && args[1].type == SCML_VAL_STRING && args[2].type == SCML_VAL_STRING) {
            return vm_ffi_call_name_args(&args[0], &args[1], &args[2], args + 3, arg_count - 3, SCML_FFI_ABI_DEFAULT, ret);
        }
        return vm_ffi_call_name_args(&args[0], NULL, NULL, args + 1, arg_count - 1, SCML_FFI_ABI_DEFAULT, ret);
    }
    if (strcmp(name, "ffi.call_name_abi") == 0 || strcmp(name, "ffi.call_by_name_abi") == 0) {
        if (arg_count < 2 || args[0].type != SCML_VAL_STRING || args[1].type != SCML_VAL_STRING) return 0;
        ScmlFFIAbi abi = scml_ffi_parse_abi(args[1].string, SCML_FFI_ABI_DEFAULT);
        if (arg_count >= 4 && args[2].type == SCML_VAL_STRING && args[3].type == SCML_VAL_STRING) {
            return vm_ffi_call_name_args(&args[0], &args[2], &args[3], args + 4, arg_count - 4, abi, ret);
        }
        return vm_ffi_call_name_args(&args[0], NULL, NULL, args + 2, arg_count - 2, abi, ret);
    }
    if (strcmp(name, "ffi.peek_ptr") == 0 || strcmp(name, "ffi.peek_pointer") == 0) {
        if (arg_count != 2) return 0;
        void *ptr = scml_ffi_peek_pointer((const void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]));
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.poke_ptr") == 0 || strcmp(name, "ffi.poke_pointer") == 0) {
        if (arg_count != 3) return 0;
        int ok = scml_ffi_poke_pointer((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), (void *)vm_value_to_pointer(&args[2]));
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.vtable_call") == 0 || strcmp(name, "ffi.com_call") == 0 || strcmp(name, "ffi.vcall") == 0) {
        if (arg_count < 4 || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING) return 0;
        ScmlFFIType arg_types[SCML_OPERANDS_MAX];
        ScmlFFISignature signature;
        if (!vm_ffi_signature_from_text(&args[2], &args[3], &signature, arg_types, sizeof(arg_types) / sizeof(arg_types[0]))) return 0;
        if (signature.arg_count != arg_count - 4) return 0;
        return scml_ffi_call_vtable((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), args + 4, &signature, SCML_FFI_ABI_DEFAULT, 1, ret);
    }
    if (strcmp(name, "ffi.vtable_call_raw") == 0 || strcmp(name, "ffi.vcall_raw") == 0) {
        if (arg_count < 4 || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING) return 0;
        ScmlFFIType arg_types[SCML_OPERANDS_MAX];
        ScmlFFISignature signature;
        if (!vm_ffi_signature_from_text(&args[2], &args[3], &signature, arg_types, sizeof(arg_types) / sizeof(arg_types[0]))) return 0;
        if (signature.arg_count != arg_count - 4) return 0;
        return scml_ffi_call_vtable((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), args + 4, &signature, SCML_FFI_ABI_DEFAULT, 0, ret);
    }
    if (strcmp(name, "ffi.vtable_call_abi") == 0 || strcmp(name, "ffi.com_call_abi") == 0) {
        if (arg_count < 5 || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING || args[4].type != SCML_VAL_STRING) return 0;
        ScmlFFIAbi abi = scml_ffi_parse_abi(args[2].string, SCML_FFI_ABI_DEFAULT);
        ScmlFFIType arg_types[SCML_OPERANDS_MAX];
        ScmlFFISignature signature;
        if (!vm_ffi_signature_from_text(&args[3], &args[4], &signature, arg_types, sizeof(arg_types) / sizeof(arg_types[0]))) return 0;
        if (signature.arg_count != arg_count - 5) return 0;
        return scml_ffi_call_vtable((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), args + 5, &signature, abi, 1, ret);
    }
    ...
    if (strcmp(name, "ffi.alloc_array") == 0) {
        if (arg_count != 2) return 0;
        size_t elem_size = 0;
        if (args[1].type == SCML_VAL_STRING) elem_size = scml_ffi_type_size(scml_ffi_parse_type(args[1].string, SCML_FFI_TYPE_INT32));
        else elem_size = (size_t)value_to_int(&args[1]);
        void *ptr = scml_ffi_alloc_array((size_t)value_to_int(&args[0]), elem_size);
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.realloc") == 0) {
        if (arg_count != 2) return 0;
        void *ptr = scml_ffi_realloc_memory((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]));
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    ...
    if (strcmp(name, "ffi.utf16") == 0 || strcmp(name, "ffi.wstring") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        void *ptr = scml_ffi_alloc_utf16(args[0].string);
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.read_utf16") == 0 || strcmp(name, "ffi.read_wstring") == 0) {
        if (arg_count < 1 || arg_count > 2) return 0;
        size_t max_units = arg_count == 2 ? (size_t)value_to_int(&args[1]) : 0;
        char *text = scml_ffi_read_utf16((const void *)vm_value_to_pointer(&args[0]), max_units);
        if (!text) return 0;
        if (ret) *ret = value_str(text);
        free(text);
        return 1;
    }
    if (strcmp(name, "ffi.write_cstring") == 0) {
        if (arg_count != 4 || args[2].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_write_cstring((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), args[2].string, (size_t)value_to_int(&args[3]));
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.track") == 0 || strcmp(name, "ffi.track_memory") == 0) {
        if (arg_count != 2) return 0;
        int ok = scml_ffi_track_memory((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]));
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.untrack") == 0 || strcmp(name, "ffi.untrack_memory") == 0) {
        if (arg_count != 1) return 0;
        int ok = scml_ffi_untrack_memory((void *)vm_value_to_pointer(&args[0]));
        if (ret) *ret = value_int(ok);
        return ok;
    }
    ...
    if (strcmp(name, "ffi.ptr_diff") == 0) {
        if (arg_count != 2) return 0;
        intptr_t diff = scml_ffi_ptr_diff((const void *)vm_value_to_pointer(&args[0]), (const void *)vm_value_to_pointer(&args[1]));
    ...
    if (strcmp(name, "ffi.alignof") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        if (ret) *ret = value_int((int32_t)scml_ffi_type_alignment(scml_ffi_parse_type(args[0].string, SCML_FFI_TYPE_INT32)));
        return 1;
    }
    ...
    if (strcmp(name, "ffi.array_read") == 0) {
        if (arg_count != 3) return 0;
        ScmlFFIType type;
        if (!vm_ffi_type_arg(&args[2], &type)) return 0;
        return scml_ffi_array_read((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), type, ret);
    }
    if (strcmp(name, "ffi.array_write") == 0) {
        if (arg_count != 4) return 0;
        ScmlFFIType type;
        if (!vm_ffi_type_arg(&args[2], &type)) return 0;
        int ok = scml_ffi_array_write((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), type, &args[3]);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    ...
    if (strcmp(name, "ffi.alloc_struct") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        void *ptr = scml_ffi_alloc_struct(args[0].string);
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.alloc_struct_array") == 0) {
        if (arg_count != 2 || args[0].type != SCML_VAL_STRING) return 0;
        void *ptr = scml_ffi_alloc_struct_array(args[0].string, (size_t)value_to_int(&args[1]));
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.struct_define") == 0) {
        if (arg_count != 2 || args[0].type != SCML_VAL_STRING || args[1].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_define_text(args[0].string, args[1].string);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_begin") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_begin(args[0].string);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.union_begin") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_union_begin(args[0].string);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_field") == 0) {
        if ((arg_count != 3 && arg_count != 4) || args[0].type != SCML_VAL_STRING || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        size_t count = arg_count == 4 ? (size_t)value_to_int(&args[3]) : 1;
        int ok = scml_ffi_struct_add_array_field(args[0].string, args[1].string, scml_ffi_parse_type(args[2].string, SCML_FFI_TYPE_INT32), count);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_finish") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_finish(args[0].string);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_size") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        if (ret) *ret = value_int((int32_t)scml_ffi_struct_size(args[0].string));
        return 1;
    }
    if (strcmp(name, "ffi.struct_align") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        if (ret) *ret = value_int((int32_t)scml_ffi_struct_alignment(args[0].string));
        return 1;
    }
    if (strcmp(name, "ffi.struct_offset") == 0 || strcmp(name, "ffi.field_offset") == 0) {
        if (arg_count != 2 || args[0].type != SCML_VAL_STRING || args[1].type != SCML_VAL_STRING) return 0;
        ScmlFFIFieldInfo info;
        if (!scml_ffi_struct_field_info(args[0].string, args[1].string, &info)) return 0;
        if (ret) *ret = value_int((int32_t)info.offset);
        return 1;
    }
    if (strcmp(name, "ffi.struct_ptr") == 0) {
        if (arg_count != 3 || args[1].type != SCML_VAL_STRING) return 0;
        void *ptr = scml_ffi_struct_ptr((void *)vm_value_to_pointer(&args[0]), args[1].string, (size_t)value_to_int(&args[2]));
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.struct_field_ptr") == 0 || strcmp(name, "ffi.field_ptr") == 0) {
        if ((arg_count != 3 && arg_count != 4) || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        void *ptr = arg_count == 4
            ? scml_ffi_struct_field_element_ptr((void *)vm_value_to_pointer(&args[0]), args[1].string, args[2].string, (size_t)value_to_int(&args[3]))
            : scml_ffi_struct_field_ptr((void *)vm_value_to_pointer(&args[0]), args[1].string, args[2].string);
        if (!ptr) return 0;
        if (ret) *ret = value_pointer((uintptr_t)ptr);
        return 1;
    }
    if (strcmp(name, "ffi.struct_read") == 0) {
        if (arg_count != 3 || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        return scml_ffi_struct_read((void *)vm_value_to_pointer(&args[0]), args[1].string, args[2].string, ret);
    }
    if (strcmp(name, "ffi.struct_write") == 0) {
        if (arg_count != 4 || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_write((void *)vm_value_to_pointer(&args[0]), args[1].string, args[2].string, &args[3]);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_field_read") == 0 || strcmp(name, "ffi.struct_field_read_index") == 0) {
        if (arg_count != 4 || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        return scml_ffi_struct_field_read_index((void *)vm_value_to_pointer(&args[0]), args[1].string, args[2].string, (size_t)value_to_int(&args[3]), ret);
    }
    if (strcmp(name, "ffi.struct_field_write") == 0 || strcmp(name, "ffi.struct_field_write_index") == 0) {
        if (arg_count != 5 || args[1].type != SCML_VAL_STRING || args[2].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_field_write_index((void *)vm_value_to_pointer(&args[0]), args[1].string, args[2].string, (size_t)value_to_int(&args[3]), &args[4]);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_array_read") == 0) {
        if (arg_count != 4 || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING) return 0;
        return scml_ffi_struct_array_read((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), args[2].string, args[3].string, ret);
    }
    if (strcmp(name, "ffi.struct_array_write") == 0) {
        if (arg_count != 5 || args[2].type != SCML_VAL_STRING || args[3].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_array_write((void *)vm_value_to_pointer(&args[0]), (size_t)value_to_int(&args[1]), args[2].string, args[3].string, &args[4]);
        if (ret) *ret = value_int(ok);
        return ok;
    }
    if (strcmp(name, "ffi.struct_undefine") == 0) {
        if (arg_count != 1 || args[0].type != SCML_VAL_STRING) return 0;
        int ok = scml_ffi_struct_undefine(args[0].string);
        if (ret) *ret = value_int(ok);
        return ok;
    }
...
int scml_vm_step(ScmlVM *vm, char *err, size_t err_size) {
    ...
    DecOp ops[SCML_OPERANDS_MAX] = {0};
    if (argc > SCML_OPERANDS_MAX) { error_at(vm, ins_pc, err, err_size, "too many operands"); return -1; }
    ...
    case SCML_OP_CALL:
        if (ops[0].type == SCML_OPERAND_ADDRESS) {
            ...
            ScmlValue args[SCML_OPERANDS_MAX - 1];
