...
static int preprocess_text(const char *path, const char *text, Macro **macros, char **out, char *err, size_t err_size){
    ...
    while(line){
        ...
        char *t=trim(raw);
        if(starts(t,"#include")){
            if(active){ char *q=strchr(t,'"'); char *r=q?strchr(q+1,'"'):NULL; if(!q||!r){snprintf(err,err_size,"bad include in %s:%d",path,line_no);goto fail;} char inc[1024]; snprintf(inc,sizeof(inc),"%s/%.*s",dir,(int)(r-q-1),q+1); char *it=read_file(inc,err,err_size); if(!it)goto fail; char *processed=NULL; if(!preprocess_text(inc,it,macros,&processed,err,err_size)){free(it);goto fail;} append(out,&olen,&ocap,processed?processed:""); free(processed); free(it);} }
        else if(starts(t,"#define")){
            if(active){ char *p=trim(t+7); char *k=p; while(*p&&!isspace((unsigned char)*p)&&*p!='(')p++; char saved=*p; if(*p)*p++=0; Macro*m=(Macro*)calloc(1,sizeof(*m)); m->name=xstrdup(k); if(saved=='('){ char *rp=strchr(p,')'); if(!rp){snprintf(err,err_size,"bad macro define in %s:%d",path,line_no); goto fail;} *rp=0; m->args=split_args(p,&m->argc); m->body=xstrdup(trim(rp+1)); } else { m->body=xstrdup(trim(p)); } m->next=*macros; *macros=m; }
        }
        else if(starts(t,"#undef")){ if(active){ macro_remove(macros, trim(t+6)); } }
        else if(starts(t,"#ifdef")){ int v=find_macro(*macros,trim(t+6))!=NULL; cond[ctop++] = (CondFrame){active, v, active&&v}; active = cond[ctop-1].active; }
        else if(starts(t,"#ifndef")){ int v=find_macro(*macros,trim(t+7))==NULL; cond[ctop++] = (CondFrame){active, v, active&&v}; active = cond[ctop-1].active; }
        else if(starts(t,"#if")){ int v=eval_expr_simple(trim(t+3),*macros); cond[ctop++] = (CondFrame){active, v, active&&v}; active = cond[ctop-1].active; }
        ...
        else if(starts(t,"#line")){ /* accepted no-op for now */ }
        else if(starts(t,"#pragma")){ /* accepted no-op for now */ }
            ...
            char lnBuf[32]; snprintf(lnBuf,sizeof(lnBuf),"%d",line_no);
            char *lineout=xstrdup(t);
            char *n=replace_all(lineout,"__FILE__",path); free(lineout); lineout=n;
            ...
            char namebuf[128]; size_t ni=0; while(lineout[ni]&&!isspace((unsigned char)lineout[ni])&&lineout[ni]!='('&&ni+1<sizeof(namebuf)){namebuf[ni]=lineout[ni];ni++;} namebuf[ni]=0;
            Macro*m=find_macro(*macros,namebuf);
            if(m && m->argc>0){ char *expanded=expand_macro_call(m,lineout); free(lineout); lineout=expanded; }
