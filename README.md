...
- SCML26 advanced surface: class objects via `useClass`, VM-known class/module/submodule registries, metadata namespaces/templates/macros, `useSub`, `getAllClassesInSubmodule`, class-name/module/method/field/tag/attribute/flag/config/dependency/derived queries, namespace-template naming, class arrays, RTTI variables, class pointers, compile-time reflection hooks, pattern matching, contracts/static assertions, inline `#embed`/`__has_embed`, placeholder bindings, safer returns, and expanded STD modules for language concepts/templates, ranges/views/algorithms, execution/concurrency, containers, memory/PMR/smart pointers, numerics/chrono/random, formatting/filesystem/text, errors/contracts, static reflection, safety, linalg/SIMD and text encoding. See [`docs/SCML26_ADVANCED_SURFACE_ES.md`](docs/SCML26_ADVANCED_SURFACE_ES.md) and [`examples/scml26_advanced_surface.scml`](examples/scml26_advanced_surface.scml).
