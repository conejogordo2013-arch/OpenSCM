...
static int ident_char(char c){ return isalnum((unsigned char)c)||c=='_'||c=='$'||c=='#'||c=='@'; }

static char *replace_all(const char *src,const char *key,const char *val){ char *out=NULL; size_t len=0,cap=0; size_t k=strlen(key); const char *p=src; while(*p){ if((p==src||!ident_char(p[-1]))&&strncmp(p,key,k)==0&&!ident_char(p[k])){ append(&out,&len,&cap,val); p+=k; } else { char tmp[2]={*p++,0}; append(&out,&len,&cap,tmp);} } return out?out:xstrdup(""); }
...
static int64_t parse_int(const char*s){ if(strlen(s)>1&&s[0]=='0'&&isxdigit((unsigned char)s[1])) return (int64_t)strtoll(s,NULL,16); return (int64_t)strtoll(s,NULL,0); }
...
static void set_integer_operand(ScmlOperand *o, const ScmlToken *tk){ o->integer=parse_int(tk->text); o->type=(o->integer<INT32_MIN||o->integer>INT32_MAX)?SCML_OPERAND_INT64:SCML_OPERAND_INT; }
